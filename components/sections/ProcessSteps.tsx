...
import { useGLTF, Environment, Html, Float, ContactShadows, PerspectiveCamera, MeshTransmissionMaterial } from "@react-three/drei";
    ...
    <group ref={group}>
      <Float speed={2} rotationIntensity={0.1} floatIntensity={0.4}>
        ...
        <Html transform distanceFactor={3} position={[0, 0, 0.06]} pointerEvents="none">
          <div className="w-[280px] p-6 bg-black/80 backdrop-blur-3xl border border-[#E0FF64]/40 rounded-[2.5rem] text-center shadow-2xl">
            <div className="text-[10px] font-black text-[#E0FF64] uppercase tracking-[0.5em] mb-2">ЭТАП 0{data.id}</div>
            <div className="text-2xl font-[900] italic uppercase text-white mb-1 tracking-tighter">{data.title}</div>
...
export const ProcessSteps = () => {
  ...
  return (
    <section id="process" className="relative h-[850px] w-full bg-transparent overflow-hidden border-y border-white/5 flex items-center justify-center">
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-20 pointer-events-none text-center w-full">
        <h2 className="text-5xl md:text-8xl font-[900] italic uppercase tracking-tighter leading-none text-neon">
