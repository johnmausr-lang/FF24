...
import { Box as BoxIcon, ScanLine, CheckCircle2, ChevronDown } from "lucide-react";
...
export const ProcessSteps = () => {
  ...
  useEffect(() => {
    if (placedSteps.length === PROCESS_STEPS.length && !isExploding) {
      setIsExploding(true);
      // Через 2.5 секунды после "взрыва" плавно скроллим к форме
      setTimeout(() => {
  ...
  const handleDragEnd = (e: any, info: any, stepId: number) => {
    // Если пользователь потянул карточку вверх (offset.y < -50) или отпустил над коробкой
    if (info.offset.y < -50) {
  ...
  return (
    <section id="process" className="py-32 relative overflow-hidden bg-slate-50 dark:bg-black transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div ref={boxRef} className="relative mx-auto mb-32 mt-10" style={{ width: size, height: size, perspective: "1500px" }}>
          ...
          >
            ...
            <div className="absolute inset-0 bg-white/10 border border-[#E0FF64]/40 backdrop-blur-xl shadow-[inset_20px_0_40px_rgba(0,0,0,0.5)]" style={{ transform: `rotateY(-90deg) translateZ(${half}px)` }} />
            <div className="absolute inset-0 bg-white/10 border border-[#E0FF64]/40 backdrop-blur-xl shadow-[inset_-20px_0_40px_rgba(255,255,255,0.05)]" style={{ transform: `rotateY(90deg) translateZ(${half}px)` }} />
            ...
            {/* Створки коробки (открываются при взрыве) */}
            <motion.div
          ...
          {/* Анимация засасывания при наведении/перетаскивании */}
        ...
        {/* ДРАГ-Н-ДРОП ЭЛЕМЕНТЫ (ЭТАПЫ) */}
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 relative z-50">
          <AnimatePresence>
            {PROCESS_STEPS.map((step, index) => {
              const isPlaced = placedSteps.includes(index);

              if (isPlaced) return null; // Если закинуто в коробку, прячем из нижнего ряда
